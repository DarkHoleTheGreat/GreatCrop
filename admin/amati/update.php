...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if(empty($nosaukums_err)){
        ...
        $sql = "UPDATE Amati SET Nosaukums=? WHERE Amats_ID=?";
         
        if($stmt = mysqli_prepare($conn, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "si", $param_nosaukums, $param_id);
    ...
    if(isset($_GET["id"]) && !empty(trim($_GET["id"]))){
        ...
        $sql = "SELECT * FROM amati WHERE Amats_ID = ?";
        if($stmt = mysqli_prepare($conn, $sql)){
            // Bind variables to the prepared statement as parameters
...
<head>
    <meta charset="UTF-8">
    <title>Atjaunināt ierakstu</title>
...
<body>
    <div class="wrapper">
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-12">
                    <h2 class="mt-5">Atjaunināt ierakstu</h2>
                    <p>Lūdzu, rediģējiet ievades vērtības un iesniedziet tās, lai atjauninātu darbinieka ierakstu.</p>
